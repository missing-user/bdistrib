...
{\ttfamily basis\_option} = 1: Use $w = 1 / (n_p |\vect{N}|)$, so the basis functions
are the Fourier functions $\sqrt{2} \sin(2\pi[mu+nv])$ and/or $\sqrt{2} \cos(2\pi[mu+nv])$ (
as determined by \parlink{symmetry\_option}).\\

{\ttfamily basis\_option} = 2: Use a constant weight, $w = 1/A$ where $A$ is the surface area. The basis functions
will be Fourier functions scaled by $\sqrt{2A/[n_p |\vect{N}|]}$.\\
}

{\ttfamily basis\_option} = 2: Use a constant weight, $w = 1/A$. The basis functions
...
{\ttfamily save\_level} = 1: Save everything except the basis functions.\\

{\ttfamily save\_level} = 2: Save everything except the basis functions and inductance matrices.\\

{\ttfamily save\_level} = 3: Save everything except the basis functions, inductance matrices, and the {\ttfamily drdu}, {\ttfamily drdv}, and {\ttfamily normal} arrays.
}
