...
for whichPlot in range(4):
    ...
    plt.plot(
        R_slice_outer[whichPlot, :], Z_slice_outer[whichPlot, :], "b.-", label="outer"
    )
    plt.plot(
...
if overlap_exists:
    ...
    plt.imshow(
        np.log10(np.abs(overlap_plasma.transpose())), interpolation="none", clim=my_clim
    )
    ...
    plt.imshow(
        np.log10(np.abs(overlap_middle.transpose())), interpolation="none", clim=my_clim
    )
    # plt.gca().xaxis.tick_top()
    plt.colorbar()
    plt.title(
...
if one_over_R_exists or const_v_exists or plasma_current_exists:
    ...
    if one_over_R_exists:
        plt.subplot(numRows, numCols, 1)
        plt.contourf(
            v_plasma, u_plasma, Bnormal_from_1_over_R_field_uv.transpose(), numContours
        )
        plt.xlabel("v", fontsize="x-small")
        plt.ylabel("u", fontsize="x-small")
    ...
    if const_v_exists:
        plt.subplot(numRows, numCols, 2)
        plt.contourf(
            v_plasma, u_plasma, Bnormal_from_const_v_coils_uv.transpose(), numContours
        )
        plt.xlabel("v", fontsize="x-small")
        plt.ylabel("u", fontsize="x-small")
        # plt.imshow(,interpolation='none')
    ...
    if plasma_current_exists:
        plt.subplot(numRows, numCols, 3)
        plt.contourf(
            v_plasma, u_plasma, Bnormal_from_plasma_current_uv.transpose(), numContours
        )
        plt.xlabel("v", fontsize="x-small")
        plt.ylabel("u", fontsize="x-small")
        # plt.imshow(,interpolation='none')
