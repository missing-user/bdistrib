...
def shouldBe(variableName, latestValue, trueValue, relativeTolerance):
...
def arrayShouldBe(
    variableName, latestValues, trueValues, relativeTolerance, requireSameLength=True
):
    ...
    if requireSameLength and (len(latestValues) != len(trueValues)):
        print(
            ...
            + " should have length "
            + str(len(trueValues))
            + " but it instead has length "
            + str(len(latestValues))
        )
        return 1
    ...
    for i in range(len(trueValues)):
        numArrayErrors += shouldBe(
            variableName, latestValues[i], trueValues[i], relativeTolerance
        )
