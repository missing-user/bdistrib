subroutine constant_v_coil_field
  ...
  ! There are 2 differences between tempVec and normal_component_of_const_v_field_uv.
  ! 1) tempVec is 1D whereas normal_component_of_const_v_field_uv is 2D.
  ...
  allocate(normal_component_of_const_v_field(num_basis_functions_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'Allocation error!'
  allocate(normal_component_of_const_v_field_inductance(num_basis_functions_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'Allocation error!'
  allocate(normal_component_of_const_v_field_transfer(num_basis_functions_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'Allocation error!'
  allocate(normal_component_of_const_v_field_uv(nu_plasma,nv_plasma),stat=iflag)
  if (iflag .ne. 0) stop 'Allocation error!'
  ...
  normal_component_of_const_v_field_uv = 0
  ...
  do iv_1 = 1, nv_plasma
     do iu_1 = 1, nu_plasma
        ...
        do iv_2 = 1, nv_middle
           do iu_2 = 1, nu_middle
              do l_2 = 0, (nfp-1)
                 ...
                 normal_component_of_const_v_field_uv(iu_1,iv_1) = normal_component_of_const_v_field_uv(iu_1,iv_1) +  &
        ...
        tempVec(index_1) = factors*normal_component_of_const_v_field_uv(iu_1,iv_1)
        normal_component_of_const_v_field_uv(iu_1,iv_1) = normal_component_of_const_v_field_uv(iu_1,iv_1) / norm_normal_plasma(iu_1,iv_1)
     end do
  end do
  !$OMP END DO
  !$OMP END PARALLEL
  ...
  normal_component_of_const_v_field_uv = normal_component_of_const_v_field_uv * prefactor
  ...
  normal_component_of_const_v_field = matmul(tempVec,basis_functions_plasma)
  call system_clock(toc1)
